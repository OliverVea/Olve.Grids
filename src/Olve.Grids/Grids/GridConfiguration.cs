...
public readonly record struct GridConfiguration(Size TileSize, (int X, int Y) Offset, int Rows, int Columns)
{
    ...
    public TileIndex GetTileIndex(int row, int column)
    {
        if (Rows == 0 || Columns == 0)
        {
            throw new InvalidOperationException("Cannot get a tile index from a grid with no rows or columns.");
        }
    ...
    public (int Row, int Column) GetRowAndColumn(TileIndex tileIndex)
    {
        if (Rows == 0 || Columns == 0)
        {
            throw new InvalidOperationException("Cannot get a row and column from a grid with no rows or columns.");
        }
        
        ArgumentOutOfRangeException.ThrowIfNegative(tileIndex.Index, nameof(tileIndex));
        ArgumentOutOfRangeException.ThrowIfGreaterThanOrEqual(tileIndex.Index, TileCount, nameof(tileIndex));
